...
contract Myth is ERC1155, Ownable {
    ...
    constructor(string memory initialURI, address _mythToken)
        ERC1155(initialURI)
    {
    ...
    function mintBatch(uint256[] memory ids, uint256[] memory amounts, address receiver) public onlyOwner {
    ...
    function initGame(
        ...
        if (action == GameAction.Start) {
            ...
            require(games[gameId].player2 == address(0) && games[gameId].player1 != address(0), "Invalid game code");
    ...
    function convertToUint256Array(uint8[10] memory numbers)
        internal
        pure
